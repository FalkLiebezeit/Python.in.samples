"""Deflection of a falling mass due to the Coriolis force.

This program simulates free fall with consideration of the Coriolis force.
"""
...
# Initial position vector [m]: starting at (0, 0, 100) meters above ground
r0 = np.array([0, 0, 100.0])
# Initial velocity vector [m/s]: starting at rest
...
# Earth's angular velocity vector [rad/s] at given latitude
...
def rhs(t, u):
    ...
    Parameters:
        t : float
            Current time (not used, as the system is time-invariant)
        u : ndarray
...
# Stop the integration when the event occurs (when z=0)
...
result = scipy.integrate.solve_ivp(
    rhs, [0, np.inf], u0, events=ground_event, dense_output=True
)
