"""Treatment of elastic collisions of particles in a box."""
...
class Mehrteilchenstoss:
    """
    A set of particles in a convex polyhedron.

        The particles consist of circles (2D) or spheres (3D) with
        a given radius and given mass. Each particle has
        a given position and a given
        initial velocity. The particles move uniformly in a straight line between
        collisions. The collisions between the
        particles and between particles and walls are completely
        elastic. No angular momentum is transferred to the individual particles.

        The boundary of the simulation domain is specified by outward-pointing
        surface normals and the corresponding distance
        from the coordinate origin.

        Using the `timestep` method, the particles are moved for
        a specified time period, and the internal
        state of the multi-particle collision is updated accordingly.

        Args:
        r (np.ndarray):
        Position vectors of the particles [m] (n_particles × n_dim).
        v (np.ndarray):
        Particle velocities [m/s] (n_particles × n_dim).
        radii (np.ndarray):
        Radii of the particles [m] (n_particles × n_dim).
        masses (np.ndarray):
        Masses of the particles [m] (n_particles).
        support (list[int]):
        Indices of the points that are support points.

        walls (tuple[np.array, np.array]):
        The first entry of the tuple contains an array
        of the distances of the walls from the coordinate origin.
        The second entry contains an array (n_walls × n_dim)
        of the outward-pointing normal vectors.
    """
