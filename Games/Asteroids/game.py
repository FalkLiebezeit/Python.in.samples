...
gunshot_sound = pygame.mixer.Sound("Python-Games/Asteroids/music/laser.wav")
explosion_sound = pygame.mixer.Sound("Python-Games/Asteroids/music/explosion.mp3")
...
backgrounds = [f'Python-Games/Asteroids/assets/background/bg{i}s.png' for i in range(1,5)]
bg = pygame.image.load(random.choice(backgrounds))

startbg = pygame.image.load('Python-Games/Asteroids/assets/start.jpg')
...
if __name__ == '__main__':
	...
	while running:
		if not gameStarted:
			if not musicStarted:
				pygame.mixer.music.load('Python-Games/Asteroids/music/Apoxode_-_Electric_1.mp3')
			...
			if not musicStarted:
				pygame.mixer.music.load('Python-Games/Asteroids/music/rpg_ambience_-_exploration.ogg')
				pygame.mixer.music.play(loops=-1)
				musicStarted = True
			for event in pygame.event.get():
				if event.type == QUIT:
